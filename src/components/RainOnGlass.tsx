...
export default function RainOnGlass() {
  ...
  useEffect(() => {
    ...
    const fragSrc = [
      ...
      'scale vec2 u_resolution;', 'uniform vec2 u_parallax;', 'uniform float u_parallaxFg;', 'uniform float u_parallaxBg;',
      ...
      '}',
    ].join('\n').replace('scale vec2 u_resolution', 'uniform vec2 u_resolution'); // Fix slight copy-paste typo if present
  ...
  return <canvas ref={canvasRef} id="rain-canvas" className="fixed inset-0" style={{ zIndex: 0, width: '100vw', height: '100vh', filter: 'saturate(0.7) brightness(0.55)' }} />;
}
